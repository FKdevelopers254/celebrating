...
import 'package:celebrate/models/celebrity.dart'; 
...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> login(String username, String password, {String? selectedRole}) async {
    final result = await AuthService.loginUser(username, password, role: selectedRole ?? _role ?? "USER");
    if (result['success']) {
      ...
      _userData = User(id: result['data']?['id'] ?? 0, username: username, email: result['data']?['email'] ?? '');
      notifyListeners();
    } else {
